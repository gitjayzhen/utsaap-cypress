...
/*
解决报错 
Uncaught SyntaxError: The following error originated from your application code, not from Cypress.
...
Cypress.on('fail', (e, test) => {
    ...
    if ((e.message).search("Cannot read property 'skip' of undefined")!= -1) {
          console.log('test "%s" is allowed to fail with error "%s"',test.title, e.message)
        
          //cy.openTab(cy.location(),{tab_name: 'myNewTab'})
    } else {
        throw e
    }
})
...
Cypress.Commands.add('openTab', (url, opts) => {
    ...
    console.info('openTab',{name_index,indexNext,active_tab_index})
    if (name_index > -1) {
